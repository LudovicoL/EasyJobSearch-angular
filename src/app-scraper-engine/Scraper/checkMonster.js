...
async function checkOffer(page, url) {
  ...
  let result = await page.evaluate(() => {
    if(document.evaluate(`//h1[@class='job_title']`,document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue != null) {
      return true;
    } else if(document.evaluate("//div[@id='NotAvailAlert' and contains(., 'Questo annuncio non è più disponibile.')]",document,null,XPathResult.FIRST_ORDERED_NODE_TYPE,null).singleNodeValue != null) {
      return false;
    }
  });
  return result;
...
async function checkMonster(date, page) {
  ...
  let links = [];
  try {
    links = await JobOfferService.getLinks(dbo, "Monster");
    ...
    console.log("Monster update: " +contaUpdate);
  } catch (error) { console.log(error); }
}
module.exports = checkMonster;
